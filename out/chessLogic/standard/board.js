...
class Board extends board_1.default {
    ...
    _pieceId = 0;
    _repitions = new Map();
    constructor(input = "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1") {
        super('standard');
        this._squares = [];
        for (let i = 0; i < 8; i++)
            this._squares.push([]);
    ...
    getFen() {
        ...
        let castlingToAdd = '';
        if (this.castleInfo.white.kingSide)
            castlingToAdd += 'K';
        if (this.castleInfo.white.queenSide)
            castlingToAdd += 'Q';
        if (this.castleInfo.black.kingSide)
            castlingToAdd += 'k';
        if (this.castleInfo.black.queenSide)
            castlingToAdd += 'q';
